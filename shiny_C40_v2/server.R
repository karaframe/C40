...
Map_data$file <- as.character(Map_data$file)

shinyServer(function(input, output) {
  
## Map and read xlsm file-------------------------------------------------------------------------------------------   
  ...
  finalMap <- reactive({
    ...
    filename <- as.character(filter(Map_data, city == input$name))[4]  #.xlsm  filename
    importWorksheets(filename)   #Table B. Summary
...
# output$contents <- renderTable({  
  ...
  # filename <- as.character(filter(Map_data, city == "Buenos Aires"))[4]  #.xlsm  filename
  ...
  # read file with the impotWorsheets function
    ...
    #    GPC_sheet <-  importWorksheets(paste(inFile$datapath, ext, sep="."))   #Table B. Summary
      ...
      # GPC_sheet <-  importWorksheets(filename)   #Table B. Summary       
